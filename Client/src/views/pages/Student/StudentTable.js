...
const StudentTable = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(API + `/api/student/${id}`, { method: "DELETE" });
      const result = await response.json();
...
const handleMetaSubmit = async () => {
    try {
      let response = await fetch(API + `/api/student/meta/${currentStudent.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      <CCol md="10">
        <CCard className="shadow-lg rounded-lg border border-gray-200">
          <CCardBody className="p-6">
            <h2 className="text-xl font-semibold text-gray-700 mb-3">Registered Students</h2>
